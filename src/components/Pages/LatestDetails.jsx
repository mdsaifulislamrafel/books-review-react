...
const LatestDetails = () => {
    ...
    const [singleLatestBook, setSingleLatestBook] = useState([]);
    useEffect(() => {
        if (latestData) {
            const singleLatestBookData = latestData.find(item => item.id === +id);
            if (singleLatestBookData) {
                setSingleLatestBook(singleLatestBookData);
            }
        }
    }, [latestData, id]);
    const { bookName, author, tags } = singleLatestBook;
    return (
        <div className=" flex items-center justify-center  flex-col md:flex-row  my-20">
            ...
            <div className="bg-white space-y-12  max-w-[350px] rounded-tr-lg rounded-br-lg md:w-[350px] text-center p-10 shadow-[0px_7px_30px_2px_rgba(100,100,111,0.2)]">
                    ...
                    <div>
                        {
                            tags?.map((tag, index) => {
                                return (
                                    <span key={index} className="inline-block px-3 py-0.5 rounded-full text-xs font-medium text-gray-700 bg-green-200">
                                        {tag}
                                    </span>
                                )
                            })
                ...
                <div>
                    <Link to="/contact" className="text-sm font-bold text-[#0d87f8] overflow-hidden shadow-lg border border-[#0d87f8] before:block before:absolute before:translate-x-full before:inset-0 before:bg-[#0d87f8] before:hover:translate-x-0 before:duration-300 before:rounded-s-full before:-z-10 after:-z-10 after:rounded-e-full after:duration-300 after:hover:translate-x-0 after:block after:absolute after:-translate-x-full after:inset-0 after:bg-[#0d87f8] relative inline-block hover:text-white py-3 px-6 rounded-full">
                        SEND MESSAGE
                    </Link>
                </div>
            </div>
